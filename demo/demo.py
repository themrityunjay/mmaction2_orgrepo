...
def parse_args():
    ...
    parser.add_argument('--config',
        default=('configs/recognition/tsn/tsn_r50_video_inference_1x1x3_100e_kinetics400_rgb.py'),
        help='spatio temporal detection config file path')
    parser.add_argument('--checkpoint',
        default=('checkpoints/tsn_r50_1x1x3_100e_kinetics400_rgb_20200614-e508be42.pth'))
    parser.add_argument('--video', help='video file/url or rawframes directory')
    parser.add_argument('--label', default='tools/data/kinetics/label_map_k400.txt',
        help='label map file')
    parser.add_argument(
        '--cfg-options',
        nargs='+',
    ...
    parser.add_argument('--out-filename', default="demo/demo_output.mp4", help='output filename')
...
def main():
    ...
    eating_labels=["eating burger","eating cake","eating carrots","eating chips",
                    "eating doughnuts","eating hotdog","eating ice cream",
                    "eating spaghetti","eating watermelon"]

    smoking_labels=["smoking","smoking hookah"]

    drinking_label=["drinking","drinking beer","drinking shots"]

    l1=str(results[0][0])

    # print(type(l1))
    if l1 in smoking_labels:
        results[0]="smoking"
    elif l1 in eating_labels:
        results[0]="eating"
    elif l1 in drinking_label:
        results[0]="drinking"
    else:
        results[0]="Unknown Action"
    ...
    if args.out_filename is not None:
        ...
        get_output(
            args.video,
            args.out_filename,
            results[0],   # previous results[0][0],
            fps=args.fps,
